...
#df['sales_year'] = df['dia_date'].year
...
dfSales = df[['dia_date','sales_year','CUSTOMER_ID','BRANDPACK_ID','Sales_Vol']]
df2016 = dfSales.query('sales_year == 2016')
print(df2016.head(10))

print(dfSales.dtypes)

# add missing dates
dfFilled2016 = df2016.set_index(['dia_date', 'BRANDPACK_ID', 'CUSTOMER_ID']).\
  unstack([1, 2]).\
  resample('D').asfreq().\
  fillna(0).\
  stack([1, 2]).\
