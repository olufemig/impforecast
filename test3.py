...
df = pd.read_csv('data/swp.csv',parse_dates=[time_column_name])

#df['sales_year'] = pd.DatetimeIndex(df['dia_date']).year
#dfSales = df[['dia_date','CUSTOMER_ID','BRANDPACK_ID','Sales_Vol']]
df['sales_year'] = pd.DatetimeIndex(df['dia_date']).year
df2016 = df.query('sales_year == 2016')
print(df2016.describe())
print(df2016.head(10))
print(df2016['sales_year'].value_counts())
print(df2016.dtypes)
